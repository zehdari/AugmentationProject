...
SOURCE_DATA_YAML = Path(r"/users/PAS2119/darklord/CVfinalproject/AugmentationProject/coco_minitrain_25k/data.yaml")
...
def write_modified_data_yaml(output_root: Path):
    ...
    #     flags=re.MULTILINE
    # )
    text = re.sub(r"^(train:\s*).*$", rf"\1{ds_name}/images/train", text, flags=re.MULTILINE)
    text = re.sub(r"^(val:\s*).*$",  r"\1Dataset/images/val",      text, flags=re.MULTILINE)
    text = re.sub(r"^(test:\s*).*$", r"\1Dataset/images/test",     text, flags=re.MULTILINE)
